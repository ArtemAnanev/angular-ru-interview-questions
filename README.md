...
**Библиотека** — это набор готовых функций и классов, которые вы можете использовать в своем коде. Вы сами решаете, когда и как использовать эти функции. Примеры библиотек: NumPy для работы с массивами в Python, или jQuery для работы с JavaScript.
 
**Фреймворк** — это более структурированная система, которая предоставляет не только набор функций, но и определенную архитектуру для вашего приложения. Фреймворк управляет потоком выполнения программы, а вы заполняете его "заготовки" своим кодом. Примеры фреймворков: Django для веб-разработки на Python, или React для создания пользовательских интерфейсов на JavaScript.
 
**Отличия:**
 
1. **Контроль**: В библиотеке вы контролируете выполнение программы, а фреймворк контролирует выполнение, предоставляя вам "точки входа".
2. **Структура**: Фреймворк предоставляет готовую структуру для приложения, в то время как библиотека просто добавляет функциональность.
3. **Использование**: Библиотеки легче интегрировать в существующий код, фреймворки требуют более значительных изменений в структуре проекта
</div>
</details>
...
1. **Bootstrap**: Один из самых популярных CSS фреймворков, который предоставляет готовые компоненты и сетку для создания адаптивных веб-сайтов.
2. **Tailwind CSS**: Утилитарный CSS фреймворк, который позволяет быстро создавать кастомизированные дизайны.
3. **Material**: Еще один мощный CSS фреймворк, который предлагает гибкие и адаптивные компоненты.
4. **Bulma**: Современный CSS фреймворк, основанный на Flexbox, который делает создание адаптивных интерфейсов простым и понятным.
 
**JavaScript библиотеки:**
 
1. **jQuery**: Одна из самых известных JavaScript библиотек, которая упрощает работу с DOM и анимациями.
2. **React**: Библиотека для создания пользовательских интерфейсов, которая использует компонентный подход.
3. **Vue.js**: Прогрессивная JavaScript библиотека, которая позволяет создавать интерактивные интерфейсы.
4. **Lodash**: Библиотека утилитарных функций, которая помогает работать с массивами, объектами и другими структурами данных.
5. **Moment.js**: Библиотека для работы с датами и временем, которая предоставляет множество полезных функций.
6. **Chart.js**: Библиотека для создания различных типов графиков и диаграмм.
 
Эти библиотеки могут значительно упростить и ускорить процесс разработки веб-приложений.
</div>
</details>
...
Когда вы вводите URL в адресную строку браузера и нажимаете Enter, браузер начинает процесс загрузки и отображения веб-страницы. Этот процесс называется Critical Rendering Path (CRP), который включает несколько ключевых шагов:
 
1. **Запрос и загрузка HTML**:
   - Браузер отправляет HTTP-запрос на сервер, чтобы получить файл `index.html`.
   - Сервер отвечает, отправляя HTML-файл обратно браузеру.
 
2. **Обработка HTML**:
   - Браузер начинает парсить (разбирать) полученный HTML-код.
   - В процессе парсинга браузер строит **DOM-дерево** (Document Object Model), которое представляет структуру документа.
 
3. **Загрузка и обработка CSS**:
   - Если в HTML-коде встречаются ссылки на CSS-файлы, браузер отправляет запросы для их загрузки.
   - После загрузки CSS-файлов браузер парсит их и строит **CSSOM-дерево** (CSS Object Model).
   - CSSOM-дерево объединяется с DOM-деревом для создания **Render Tree**, который содержит только видимые элементы и их стили.
 
4. **Расчет макета (Layout)**:
   - Браузер вычисляет размеры и позиции каждого элемента в Render Tree.
   - Этот процесс называется **layout** или **reflow**, и результатом является **layout tree**.
 
5. **Отрисовка (Painting)**:
   - Браузер отрисовывает элементы на экране, используя информацию из layout tree.
   - Процесс отрисовки включает **painting** (заполнение цветом, градиенты и т.д.) и **compositing** (объединение различных слоев).
 
6. **Загрузка и выполнение JavaScript**:
   - Если в HTML-коде есть скрипты, браузер загружает и выполняет их.
   - JavaScript может изменять DOM и CSSOM, что может привести к повторному выполнению шагов layout и painting.
 
Важно отметить, что современные браузеры стараются выполнять эти шаги параллельно и оптимизировать процесс, чтобы ускорить отображение страницы. Например, браузеры могут начинать отрисовку частей страницы до полной загрузки всех ресурсов, что позволяет пользователю видеть контент быстрее.
 
Оптимизация Critical Rendering Path важна для улучшения производительности веб-страницы, так как она напрямую влияет на время загрузки и отображения контента для пользователя.
...
3. **BigInt**:
   - Представляет целые числа произвольной длины.
...
5. **Undefined**:
   - Тип данных, присваиваемый переменным, которым не было явно назначено значение.
   - Пример: `let x; // x будет undefined`.
 
6. **Null**:
   - Представляет "ничто" или "пустое значение".
   - Пример: `let y = null;`.
 
7. **Symbol** (символ):
   - Используется для создания уникальных идентификаторов.
   - Пример: `const sym = Symbol('description');`.
 
### Объектные типы данных
 
8. **Object**:
   - Это сложный тип данных, который может хранить коллекции и более сложные структуры.
   - Пример: 
     ```javascript
     const person = {
         name: 'Alice',
...
9. **Array**:
   - Специальный тип объекта, хранящий упорядоченные коллекции данных.
   - Пример: 
     ```javascript
...
10. **Function**:
    - Функции в JavaScript также являются объектами. Это первый класс, что означает, что их можно передавать как аргументы, возвращать из других функций и хранить в переменных.
    - Пример:
      ```javascript
      function greet() {
          console.log('Hello!');
...
- В JavaScript, тип данных можно проверить с помощью оператора `typeof`, например: `typeof "Hello"` вернет `"string"`.
- Примитивные типы данных являются неизменяемыми (immutable), что означает, что вы не можете изменять значение переменной. Вместо этого вы создаете новую переменную с новым значением.
- Объекты являются изменяемыми (mutable), и вы можете изменять их свойства и значения после создания.
 
Эти типы данных предоставляют гибкие возможности для работы с информацией в JavaScript!
